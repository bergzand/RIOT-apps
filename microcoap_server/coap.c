...
static int handle_get_saul_temp(coap_rw_buffer_t *scratch,
                                 const coap_packet_t *inpkt,
                                 coap_packet_t *outpkt,
                                 uint8_t id_hi, uint8_t id_lo);
...
static const coap_endpoint_path_t path_saul_temp =
        { 2, { "saul", "temp1" } };
...
const coap_endpoint_t endpoints[] =
{
    ...
    { COAP_METHOD_GET,	handle_get_saul_temp,
        &path_saul_temp,	   "ct=0"  },
        ...
        uint8_t id_hi, uint8_t id_lo)
{
    ...
    int payload_len = fmt_s16_dfp((char *)&response, phy.val[0],2);
    return coap_make_response(scratch, outpkt, (const uint8_t *)response, payload_len,
                              id_hi, id_lo, &inpkt->tok, COAP_RSPCODE_CONTENT,
                              COAP_CONTENTTYPE_TEXT_PLAIN);
}
