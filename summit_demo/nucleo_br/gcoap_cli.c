...
static ssize_t _app_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len);
static ssize_t _os_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len);
static ssize_t _board_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len);
static ssize_t _mcu_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len);

/* CoAP resources */
static const coap_resource_t _resources[] = {
    { "/board", COAP_GET, _board_handler },
    { "/cli/stats", COAP_GET, _stats_handler },
    { "/mcu", COAP_GET, _mcu_handler },
    { "/name", COAP_GET, _app_handler },
    { "/os", COAP_GET, _os_handler },
    { "/etx", COAP_GET, _count_handler },
    { "/tx", COAP_GET, _count_handler },
};
...
static ssize_t _mcu_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len)
{
    ...
    const char *board_name = RIOT_MCU;
    size_t payload_len = strlen(RIOT_MCU);
    memcpy(pdu->payload, board_name, payload_len);
...
{
    gcoap_resp_init(pdu, buf, len, COAP_CODE_CONTENT);
    const char *board_name = RIOT_BOARD;
...
{
    gcoap_resp_init(pdu, buf, len, COAP_CODE_CONTENT);
    const char *os_name = "riot";
    size_t payload_len = strlen("riot");
    memcpy(pdu->payload, os_name, payload_len);
    return gcoap_finish(pdu, payload_len, COAP_FORMAT_TEXT);
}
